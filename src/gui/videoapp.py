...
class VideoApp:
    ...
    def show_progress_bar(self):
        ...
        self.progress_bar = ctk.CTkProgressBar(self.progress_popup, orient="horizontal", length=300, mode="determinate")
    ...
    def create_widgets(self):
            ...
            self.video_view = ctk.CTkCanvas(self.video_frame, width=640, height=480)
            ...
            self.slider = ctk.CTkSlider(self.video_frame, from_=0, to=100, orientation="horizontal", width=400,
                                        number_of_steps=1, fg_color="red", progress_color="green",
                                        button_color="yellow")
            ...
            # self.filter_frame2 = ctk.CTkFrame(self.root)

            # self.filter_frame2.pack(side=ctk.RIGHT, fill=ctk.Y, padx=10, pady=10)

            # self.open_button = ctk.CTkButton(self.filter_frame2, text="Ass", command=self.open_video)
            # self.open_button.pack(pady=10)
    ...
    def show_crop_interface(self, frame):
        ...
        canvas = ctk.CTkCanvas(top, width=display_img.width, height=display_img.height)
        canvas.pack()
        ...
        skip_button = ctk.CTkButton(top, text="Skip Cropping", command=skip_cropping)
        skip_button.pack(side=ctk.BOTTOM, pady=10)
    ...
    def show_filter_popup(self):
        ...
        self.filter_vars = {filter_name: ctk.BooleanVar() for filter_name in filters}
        
        for filter_name in filters:
            frame = ctk.CTkFrame(popup)
            frame.pack(anchor='w', fill='x')
            
            chk = ctk.Checkbutton(frame, text=filter_name, variable=self.filter_vars[filter_name])
            chk.pack(side=ctk.LEFT, anchor='w')
            
            info_button = ctk.CTkButton(frame, text="i", command=lambda fn=filter_name: self.show_filter_preview(fn))
            info_button.pack(side=ctk.RIGHT, padx=5)

        apply_button = ctk.CTkButton(popup, text="Apply", command=lambda: self.apply_filters(popup))
    ...
    def show_filter_preview(self, filter_name):
        ...
        preview_popup = ctk.CTkToplevel(self.root)
        ...
        canvas = ctk.ctk.CTkCanvas(preview_popup, width=gif.width, height=gif.height)
        canvas.pack()
    ...
    def show_tracked_points_table(self):
        ...
        table_popup = ctk.CTkToplevel(self.root)
        table_popup.title("Tracked Points Table")
        table_frame = ctk.CTkFrame(table_popup)
        ...
        export_button = ctk.CTkButton(table_popup, text="Export as CSV", command=self.export_tracked_points_to_csv)
    ...
    def choose_tracking_method(self):
        ...
        ctk.Radiobutton(method_popup, text="Track Points", variable=self.tracking_method, value="points").pack(anchor=ctk.W)
        ctk.Radiobutton(method_popup, text="Track Centroid", variable=self.tracking_method, value="bbox").pack(anchor=ctk.W)
    ...
    def calculate_and_export_derivative(self, order):
        ...
        ctk.CTkButton(popup, text="Plot", command=plot_derivative).pack(pady=5)
        ctk.CTkButton(popup, text="Export", command=export_derivative).pack(pady=5)
    ...
    def show_point_selection_popup(self):
        self.point_selection_popup = ctk.CTkToplevel(self.root)
        self.point_selection_popup.title("Select Two Points")

        ctk.CTkLabel(self.point_selection_popup, text="Select two points:").pack(pady=10)
        ...
        for i, point in enumerate(self.processor.points_to_track):
            ...
            chk.pack(anchor=ctk.W)
            self.point_vars.append(var)

        ctk.CTkButton(self.point_selection_popup, text="Confirm", command=self.confirm_selected_points).pack(pady=10)
    ...
    def track_angle_between_points(self):
        ...
        ctk.CTkButton(popup, text="Visualize Angles", command=self.visualize_angles).pack(pady=5)
        ctk.CTkButton(popup, text="Export Angles to CSV", command=self.export_angles_to_csv).pack(pady=5)
    ...
    def start_tracking(self):
        ...
        self.display_clipped_frames()
        self.track_coords_button.config(state=ctk.NORMAL)  # Enable coordinates button
