...
class ScaleRuler:
    ...
    def onclick(self, event):
        ...
        if abs(x - self.p1[0]) < 4 and abs(y - self.p1[1]) < 4:
            self.dragging = "resize1"
        
        elif abs(x - self.p2[0]) < 4 and abs(y - self.p2[1]) < 4:
            self.dragging = "resize2"
        
        elif self.p1[0]+4 < x < self.p2[0]-4 and self.p1[1] - self.height < y < self.p1[1] + self.height:
            self.dragging = "move"
            self.offset = [x - self.p1[0], y - self.p1[1]]

    def ondrag(self, event):
        if self.dragging == "resize1":
            if (self.p2[0] - event.x) > self.width:
                self.p1[0] = event.x
        
        elif self.dragging == "resize2":
            if (event.x - self.p1[0]) > self.width:
                self.p2[0] = event.x
